  ...
  return (
    <div className="mt-8 animate-fade-in-up space-y-6">
      ...
      <div className="bg-white/5 backdrop-blur-lg rounded-2xl border border-white/10 overflow-hidden">
        <div className="bg-purple-500/20 px-6 py-4 border-b border-white/10">
          ...
          <p className="text-slate-400 text-sm mt-1">
            {cartItems.reduce((sum, item) => sum + item.quantity, 0)} items to
            optimize
          </p>
        </div>
          ...
          <div className="p-5 space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-white font-medium">
                ₹{result.recommended.feeBreakdown.productCost}
              </span>
            </div>
      ...
      <div className="bg-gradient-to-br from-green-600 to-emerald-700 rounded-2xl p-6 text-center shadow-2xl">
        <p className="text-green-100 text-sm uppercase tracking-wide mb-1">
          You Pay
        </p>
      ...
      {hasSavings &&
        (() => {
          const savingsPercentage =
            (result.savings / result.recommended.totalCost) * 100;
      ...
      {result.alternatives && result.alternatives.length > 1 && (
        <AlternativesComparison
          ...
          currentBest={{
            platform: result.recommended.platform || "",
            totalCost: result.recommended.totalCost,
            productCost: result.recommended.productCost || 0,
            feeBreakdown: result.recommended.feeBreakdown,
          }}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {result.missingProducts.map((product) => (
                  <span
                    key={product}
                    className="px-2 py-1 bg-red-500/30 rounded-lg text-xs text-red-200"
                  >
                    {product}
                  </span>
                ))}
